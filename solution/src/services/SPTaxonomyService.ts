...
export default class SPTaxonomyService {
    ...
    private async getFormDigest(): Promise<string> {
        ...
        let contextInfoResponse: SPHttpClientResponse = await this.context.spHttpClient.post(
            this.contextInfoUrl, SPHttpClient.configurations.v1, httpPostOptions);
    ...
    public async getTermsFromTermSet(termSetName: string, locale: number = 1033): Promise<ITerm[]> {
        ...
        Environment.type === EnvironmentType.ClassicSharePoint) {
            ...
            let serviceResponse: SPHttpClientResponse = 
                await this.context.spHttpClient.post(this.clientServiceUrl, SPHttpClient.configurations.v1, httpPostOptions);
            let serviceJSONResponse: Array<any> = await serviceResponse.json();

            let result: Array<ITerm> = new Array<ITerm>();

            // Termsetcollection extraction
        ...
        // Empty array in case of missing data
